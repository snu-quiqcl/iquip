...
from iquip.protocols import ExperimentInfo, ConfigurationInfo
...
class ConfigurationInfoThread(QThread):
    """QThread for obtaining the experiment information from the proxy server.
    ...
    def run(self):
        ...
        """
        try:
            response = requests.get(f"http://{self.ip}:{self.port}/configuration/info/",
                                    params={"file": self.configurationPath},
                                    timeout=10)
            response.raise_for_status()
            data = response.json()
        except requests.exceptions.RequestException:
            ...
            return
        if data:
            configurationInfos: Dict[str, ConfigurationInfo] = {}
            for cls, info in data.items():
                configurationInfos[cls] = ConfigurationInfo(**info)
            self.fetched.emit(configurationInfos)
        else:
            logger.info("The selected item is not an experiment file.")
